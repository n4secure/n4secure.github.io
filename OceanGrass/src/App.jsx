...
import { 
  ...
  ArrowRight, Award, MapPin, Share2, X, ExternalLink, Turtle, FileText
} from 'lucide-react';
import { 
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer 
} from 'recharts';

// --- Data for Research Modals ---
const RESEARCH_DETAILS = [
  {
    ...
    fullContent: (
      <div className="space-y-4">
        <p>
          <strong className="text-teal-700">ปัญหา:</strong> การนำหญ้าทะเลจากธรรมชาติมาปลูกฟื้นฟูมักมีอัตรารอดต่ำและเป็นการรบกวนแหล่งหญ้าเดิม
        </p>
        <p>
          <strong className="text-teal-700">ทางออก:</strong> ทีมวิจัยจากคณะประมง มหาวิทยาลัยเกษตรศาสตร์ นำโดย รศ.ชัชรี แก้วสุรลิขิต ประสบความสำเร็จในการพัฒนาเทคนิค <strong>"เพาะเลี้ยงเนื้อเยื่อหญ้าชะเงาใบยาว" (Enhalus acoroides)</strong>
        </p>
  ...
  },
  {
    ...
    fullContent: (
      <div className="space-y-4">
        <p>
          <strong className="text-blue-700">เทคโนโลยี:</strong> การบูรณาการระหว่างภาพถ่ายดาวเทียม Sentinel-2 และภาพถ่ายจากโดรน (UAV) เพื่อสำรวจแนวหญ้าทะเล
        </p>
        <p>
          <strong className="text-blue-700">พื้นที่ศึกษา:</strong> อ่าวขาม อุทยานแห่งชาติหาดเจ้าไหม และพื้นที่ภาคตะวันออก (จ.ตราด)
        </p>
        <ul className="list-disc pl-5 text-gray-600 space-y-2">
  ...
  },
  {
    ...
    fullContent: (
      <div className="space-y-4">
        <p>
          <strong className="text-green-700">โมเดลความสำเร็จ:</strong> การอนุรักษ์ที่ยั่งยืนที่สุดคือการให้ "ชุมชน" เป็นเจ้าของ
        </p>
        <p>
          <strong className="text-green-700">กรณีศึกษา:</strong> บ้านพรุจูด จ.ตรัง และ ชุมชนชายฝั่ง จ.ตราด
        </p>
        <ul className="list-disc pl-5 text-gray-600 space-y-2">
...
const Hero = ({ onStart }) => (
    <div className="relative min-h-screen flex items-center justify-center wave-bg overflow-hidden pt-16">
        ...
        <div className="max-w-4xl mx-auto text-center px-4 z-10 text-white fade-in">
            {/* Added School Logo in Hero Section */}
            <img 
                src="https://rayongwit.ac.th/wp-content/uploads/2023/04/cropped-Logo-%E0%B9%80%E0%B8%A7%E0%B9%87%E0%B8%9A-%E0%B8%A3%E0%B8%A3-V3.png" 
                alt="โรงเรียนระยองวิทยาคม" 
        ...
        <div className="max-w-5xl mx-auto px-4 py-20 animate-fade-in">
            ...
            <div className="grid md:grid-cols-2 gap-12 items-center mb-20">
                ...
                <div className="space-y-6">
                    ...
                    {/* ส่วนที่เพิ่มใหม่สำหรับเต่าทะเล */}
                    <div className="flex gap-4 p-4 rounded-xl hover:bg-white hover:shadow-lg transition-all cursor-default">
...
const AboutUs = () => {
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-20 animate-fade-in">
            <div className="text-center mb-16">
                <span className="text-teal-600 font-semibold tracking-wider text-sm uppercase">Our Team & Research</span>
            ...
            <div className="mb-20">
                <h3 className="text-2xl font-bold text-gray-800 mb-8 flex items-center gap-2">
                    <Users className="w-6 h-6 text-teal-600" /> ทีมงานของเรา
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {/* Placeholder Team Members */}
                    {[1, 2, 3].map((item) => (
                        <div key={item} className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 flex flex-col items-center text-center hover:shadow-xl transition-shadow">
                            <div className="w-24 h-24 bg-gray-200 rounded-full mb-4 overflow-hidden border-4 border-teal-50">
                                <div className="w-full h-full bg-teal-100 flex items-center justify-center text-teal-500">
                                    <Users className="w-10 h-10" />
                                </div>
                            </div>
                            <h4 className="text-lg font-bold text-gray-800">ชื่อสมาชิก {item}</h4>
                            <p className="text-sm text-gray-500">ตำแหน่ง / หน้าที่</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div>
                <h3 className="text-2xl font-bold text-gray-800 mb-8 flex items-center gap-2">
                    <BookOpen className="w-6 h-6 text-blue-600" /> งานวิจัยและรายงาน
                </h3>
                <div className="grid gap-4">
                    {researchPapers.map((paper, index) => (
                        <div key={index} className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex items-start gap-4 hover:shadow-md transition-shadow hover:border-teal-200 cursor-pointer">
                            <div className="bg-red-50 p-3 rounded-lg text-red-500 shrink-0">
                                <FileText className="w-6 h-6" />
                            </div>
                            <div>
                                <h4 className="font-semibold text-gray-800 text-lg leading-tight">{paper}</h4>
                                <p className="text-sm text-gray-500 mt-2 flex items-center gap-2">
                                    <span className="bg-gray-100 px-2 py-0.5 rounded text-xs font-medium">PDF</span>
